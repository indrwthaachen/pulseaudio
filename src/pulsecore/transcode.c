...
#define OPUS_FRAME_SIZE 2880
#define OPUS_SAMPLE_RATE 48000
#define OPUS_SAMPLE_SIZE 2
#define OPUS_BITRATE 64000


OpusEncoder *tc_opus_create_encoder(int sample_rate, int channels);
...
void pa_transcode_init(pa_transcode *transcode, pa_encoding_t encoding, pa_transcode_flags_t flags, pa_format_info *transcode_format_info, pa_sample_spec *transcode_sink_spec) {
         ...
         switch(encoding) {
                  ...
                  case PA_ENCODING_OPUS:
                           ...
                           transcode->max_frame_size = OPUS_FRAME_SIZE;
                           transcode->sample_size = OPUS_SAMPLE_SIZE;
                           transcode->frame_size = OPUS_FRAME_SIZE;
                           ...
                           if(flags & PA_TRANSCODE_DECODER) {
                                    ...
                                    transcode->encoder = tc_opus_create_encoder(OPUS_SAMPLE_RATE, transcode_sink_spec->channels);
                                    transcode->channels = transcode_sink_spec->channels;
                                    transcode->rate = OPUS_SAMPLE_RATE;
